...
class UserRepositoryTest {
    ...
    fun givenValidToken_whenRetrievingUserData_shouldUpdateUserLiveData() {
        ...
        Truth.assertThat(userRepository.userLiveData.value?.fullname).isEqualTo("le name")
    }
    ...
    fun givenValidParameter_whenUpdatingProfileData_shouldUpdateStatusCodeLiveData() {
        ...
        verify(userService, times(1))
            .updateProfileData("token", updateProfileRequest)
        verify(basicCall, times(1))
            .enqueue(any(UserRepository.BasicCallbackHandler::class.java))

        Truth.assertThat(userRepository.editProfileStatusCode.value).isEqualTo(200)
    }
    ...
    fun givenValidCookie_whenLoggingIn_shouldUpdateTokenLiveData() {
        ...
        verify(tokenCall, times(1))
            .enqueue(any(UserRepository.TokenCallbackHandler::class.java))
        Truth.assertThat(userRepository.tokenLiveData.value).isEqualTo("le token")
    }
}