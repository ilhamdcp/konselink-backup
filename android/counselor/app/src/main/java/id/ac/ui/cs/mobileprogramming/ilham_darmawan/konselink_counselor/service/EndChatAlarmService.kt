...
import id.ac.ui.cs.mobileprogramming.ilham_darmawan.konselink_counselor.model.socketio.EndChatRequest
            ...
            if (it.status!! == ConsultationStatus.CHAT.code) {
                db.consultationDao().insertOrUpdateConsultation(
                    Consultation(
                        ...
                        status = ConsultationStatus.FILL_RECORD.code
                    )
                )
            } else if (it.status!! == ConsultationStatus.FILL_RECORD.code) {
                db.consultationDao().insertOrUpdateConsultation(
                    Consultation(
                        scheduleId = scheduleId,
