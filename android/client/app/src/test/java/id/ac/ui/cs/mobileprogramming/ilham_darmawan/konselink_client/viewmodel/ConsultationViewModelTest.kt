...
class ConsultationViewModelTest {
    ...
    fun setUp() {
        ...
        consultationViewModel = ConsultationViewModel(activity, consultationService, db)
    ...
    @After
    fun tearDown() {
        verifyNoMoreInteractions(consultationService, db, ongoingCall, chatHistoryCall, basicCall)
    }
    ...
    fun givenValidToken_whenRetrievingOngoingSchedule_shouldUpdateLiveData() {
        ...
        verify(ongoingCall, times(1))
            .enqueue(any(ConsultationRepository.OngoingConsultationCallbackHandler::class.java))
        verify(consultationService, times(1)).retrieveOngoingSchedule("token")
        verify(db, times(1)).consultationDao()

        assertThat(consultationViewModel.ongoingScheduleLiveData.value?.counselorName).isEqualTo("Macross")
    }
    ...
    fun givenValidToken_whenRetrievingUpcomingSchedule_shouldUpdateLiveData() {
        ...
        verify(upcomingCall, times(1))
            .enqueue(any(ConsultationRepository.UpcomingConsultationCallbackHandler::class.java))
        verify(consultationService, times(1)).retrieveUpcomingSchedule("token", 1 , 1)
        verify(db, times(1)).consultationDao()

        assertThat(consultationViewModel.upcomingScheduleLiveData.value?.counselorName).isEqualTo("Macross")
    }

    @Test
    fun givenNonFilledPanasScale_whenValidatingPreConsultationPage_shouldReturnFalse() {
        verify(db, times(1)).consultationDao()
        assertThat(consultationViewModel.preConsultationPageIsValid()).isFalse()
    }

    @Test
    fun givenFilledPanasScale_whenValidatingPreConsultationPage_shouldReturnTrue() {
        verify(db, times(1)).consultationDao()
        consultationViewModel.panasValues.keys.forEach {
            consultationViewModel.panasValues[it] = 5
        }

        assertThat(consultationViewModel.preConsultationPageIsValid()).isTrue()
    }
    ...
    fun givenValidTokenAndValidPreConsultationSurvey_whenPostingPreConsultationSurvey_shouldUpdateStatusCodeLiveData() {
        ...
        `when`(consultationService.postPreConsultationSurvey(anyString(), any(
            PreConsultationSurveyRequest::class.java))).thenReturn(basicCall)
        doAnswer {
        ...
        consultationViewModel.postPreConsultationData("token", 1)
        verify(basicCall, times(1)).enqueue(any(UserRepository.BasicCallbackHandler::class.java))
        verify(consultationService, times(1)).postPreConsultationSurvey(anyString(), any(
            PreConsultationSurveyRequest::class.java))
        verify(db, times(1)).consultationDao()

        assertThat(consultationViewModel.postPreConsultationCodeLiveData.value).isEqualTo(200)
    }
